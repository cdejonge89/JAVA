    ...
    class MinHeap {
         ...
         */
    insert(num) {
        // inserting new node at the end of the heap array
        this.heap.push(num)
        // finding correct position
        if(this.heap.length > 1);
        let current = this.heap.length-1;
        // traversing up the parent node until the current doe is greater than the parent
        while(current > 1 && this.heap[Math.floor(current/2)] > this.heap[current]){
            ...
            // [this.heap[Math.floor(current/2)], this.heap[current]] = [this.heap[current], current = Math.floor(current/2)]
        }
    }
